...
| M5-3 | client_msg_id UUID v4 + process startup rehydrate | F-MEDIA-01 / § 10 ARCH-DESIGN | ✅ 已完成 (bundled into M5-2 commit `bf52d90` per S40.0 scope recombination · **no separate M5-3 commit**) — owns the two files: `src/lib/db/client_msg_id.ts` UUID v4 helper (`generateClientMsgId()` + `isValidClientMsgId` regex) + `src/lib/db/rehydrate.ts` in-flight outbox rehydrate on app boot. **Idempotent double-replay safety** = (a) server DB partial unique index `messages_client_msg_id_unique_idx` + (b) Workbox SW bg sync replay (see M5-2 row above) — dedupe even under partial-network-recovery. 0 new tsc errors. 本机 static-only per KI-9. |
