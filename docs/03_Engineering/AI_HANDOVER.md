...
| **代码版本** | `v0.5.0 + M3-1 (c88c076) + M3-2 (75d7300) + M3-3 (70d6e41) + M3-4 (0fa9cbd) + M3-5 (d1915f2) + M4-1/2 (d39dbbe) + M4-3 (9fa1968) + M4-4 (ea2f2ef) + M4-5 (7f2b47a) + M4-6 (33e4179 + fixup 6eaa861) + M4-7 (0111398 + RT closure 075b4b1 + 540165a) + M4-7.1 polish (7e3ec3f) + M4-8 (d320483) + M5-1 (dadcb01) + M5-2 (bf52d90) + M5-3 bundled per S40.0 + M5-4 (d6c0ae2) + M5-5 (5e7fab3) + M5-6 (75c286e) · S40.0 batch · annotated tag `v0.5.0+M5.6` · + **M5-7 (6e593f2) · S41.0 docs sync · annotated tag `v0.5.0+M5.7`**` · + **M6-4 (85a57e9) · S42.0 docs sync · annotated tag `v0.5.0+M6.4`** · + **M6-5 + M6-6 + M6-7 (S43.0) — admin-delete-friend EF + ConfirmModal + copy-to-clipboard · 33 files · 398 tests · 0 new tsc errors · annotated tag `v0.5.0+M6`** |
| **目标用户** | Owner（1）+ Friend（≤ 20 社交目标） |
| **MVP 节拍** | 4-6 周个人工作量 |
| **产品定位** | "少数密友的数字避难所"（不商业化 / 不规模化） |
| **Next session** | **M7-4 Responsive layout (F-UI-01 / NF-RESP-N01)** — Sidebar→drawer on <1024px · ChatPanel 适配 · 3 断点。**M6 batch fully complete**: M6-1..7 all ship 走了 · annotated tag `v0.5.0+M6` created. **Deferred v1.1+**: M5-4-compress canvas WebP compression · M5-2.1 manual retry button · M6-4.1 friend-side `/reset-password/:token` completion EF · M5-1.1 quota UI · push-notification cross-device sync |
...
| ✅ 已完成 | **M6-5 + M6-6 · admin-delete-friend EF + ConfirmModal (CAP-20 / F-SEC-06 / BF-14 / AC.18)** | **M6-5**: `fn_admin_delete_friend` RPC (SECURITY DEFINER · FOR UPDATE row lock · SET `profiles.deleted_at` + `conversation_members.left_at` atomic · idempotent re-call) + migration 0018 `profiles.deleted_at` ADD COLUMN + partial indexes + GRANT service_role · EF `admin-delete-friend` (JWT-verified · 3-layer defense) · `admin.ts` `deleteFriend()` + `DeletedFriendSummary` · `DeleteFriendCard` (friend picker + open confirm modal + success card + error strip) · `useDeleteFriend` mutation with cache invalidation
**M6-6**: Reusable `<ConfirmModal>` via `createPortal` · phrase gate (case-insensitive trim match) · Escape/cancel/backdrop-deny-close · `aria-modal` + Tab-trap · `testIdPrefix` isolation
**Verification**: 33 files · 398 tests passed · 0 new tsc errors · 1 reviewer should-fix (useCallback→useMemo in ConfirmModal.tsx) APPLIED
**Deferred**: M6-4.1 friend-side completion EF (v1.1+) |
| 🟢 下一步 | **M6-7 copy invite URL helper** | M6-5 + M6-6 ship 走了 (S43.0) · M6-7 = `navigator.clipboard.writeText` copy-to-clipboard on invite URL creation · closes M6 batch · M6-end-batch annotated tag `v0.5.0+M6` after M6-7 ship. **Deferred v1.1+**: M5-4-compress canvas WebP compression · M5-2.1 manual retry button · M6-4.1 friend-side `/reset-password/:token` completion EF · M5-1.1 quota UI |
