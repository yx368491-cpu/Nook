...
| M5-2 | Workbox SW bg sync + useSendMessage outbox rewire (M5-3 rehydrate bundled) | F-MEDIA-01 | ✅ 已完成 (commit `bf52d90` · S35.0 · 本机 static-only per KI-9) — ship dependency from M5-1 (Dexie outbox foundation · `applyMark*` api) directly into the send path. Cross-ref: see **M5-1 row** for Dexie/outbox state types; see **M5-3 row below** for `src/lib/db/client_msg_id.ts` + `src/lib/db/rehydrate.ts` (bundled into this commit per S40.0 scope recombination). 11/11 vitest M5-2 specs + 159/159 full unit suite + 0 new tsc errors + 0 critical reviewer blockers. M5-2.1 followup = manual 「重试」 button + outbox in-app toast (deferred v1.1+). 1 runtime dep `workbox-window@^7` + devDep `vite-plugin-pwa@^0.x`. |
| M5-3 | client_msg_id UUID v4 + dedupe + process startup rehydrate | F-MEDIA-01 / § 10 ARCH-DESIGN | ✅ 已完成 (bundled into M5-2 commit `bf52d90` per S40.0 scope recombination · **no separate M5-3 commit**) — owns the two files: `src/lib/db/client_msg_id.ts` UUID v4 helper (`generateClientMsgId()` + `isValidClientMsgId` regex · replaces inline `crypto.randomUUID()` calls in `Composer.tsx` per M3-4) + `src/lib/db/rehydrate.ts` in-flight outbox rehydrate on app boot. **Idempotent double-replay safety** = (a) server DB partial unique index `messages_client_msg_id_unique_idx` + (b) Workbox SW bg sync replay (see M5-2 row above) — dedupe even under partial-network-recovery. **5 `client_msg_id` cases in `src/lib/db/client_msg_id.test.ts` + 4 M5-2 `useSendMessage` cases in `src/hooks/useSendMessage.test.tsx` share the id-gen path** (vitest regression-range attribution for M5-2.1 followup). 0 new tsc errors. 本机 static-only per KI-9. |
...
| M5-7 | 直传 50MB 文件（Supabase Storage signed URL） + UI 进度条 + drag-drop affordance | F-MSG-03 | ✅ 已完成 (commit `6e593f2` · S41.0 · 本机 static-only) — ❶ `chat.ts` 扩 `UploadAttachmentOptions` interface (`onProgress?: (loaded, total) => void` + `signal?: AbortSignal`) + `AttachmentUploadError` class (code: STORAGE_ERROR / NETWORK_ERROR / CANCELLED / AUTH_MISSING) + `uploadAttachmentBytes` XHR-direct helper (Bearer auth from `supabase.auth.getSession()` snapshot · POST `${env.supabaseUrl}/storage/v1/object/attachments/<path>` · `xhr.upload.onprogress` drives UI · `xhr.abort()` bridges signal cancel · `signal.aborted===true` short-circuit throws CANCELLED · `{ once: true }` listener) + extended `uploadAttachment` + `sendAttachmentMessage` opts passthrough (SDK path register when opts absent) + import `env` ❷ `useSendMessage.ts` ❸ `useFileUploadProgress.ts` (NEW · -start(c/Signal)/cancel=AbortController.abort/reset=clearState · cleanup on unmount） ❹ `UploadProgressBar` (NEW · role=progressbar + aria-valuen{now/min/max} + cancel button) ❺ `AttachmentDropZone` (NEW · pointer-events-none dashed-soft overlay · icon + title + hint) ❻ `Composer.tsx` dispatchFile startUpload·mutateAsync{onProgress,signal}·finally resetUpload·CANCELLED silencing in catch BEFORE existing cascade ❼ `tokens.css` @keyframes progress-fade-in (120 ms ease-out, honors prefers-reduced-motion) ❽ i18n × 2 lang: `chat.upload.{progress,progressAria,cancelAria}` + `chat.dropZone.{title,hint}` ❾ Tests 18 NEW: hook 8 + ProgressBar 6 + DropZone 4 · vitest 253/253 + 0 new tsc errors + reviewer ship-ready after 1 should-fix (zh-CN `progressAria` 单 brace→双 brace i18next interpolation)
...
| M6-4 | EF `admin-reset-password` (CAP-19 / F-AUTH-07 / AC.16) | 13 files ship | ✅ 已完成 (commit `85a57e9` · S42.0 · 本机 static-only per KI-9) — ❶ `supabase/migrations/20260628000017_admin_reset_password.sql` (NEW · ~85 行) — extends invites：ALTER CHECK 包含 `'password_reset'` · nullable `target_user_id` UUID FK → auth.users · partial index on `target_user_id WHERE target_kind='password_reset'` · partial UNIQUE index `(target_user_id) WHERE target_kind='password_reset' AND used_at IS NULL AND revoked_at IS NULL` 防 double-pending · consistency CHECK (password_reset ↔ target_user_id NOT NULL) ❷ `supabase/functions/admin-reset-password/index.ts` (NEW · ~190 行) — JWT-verified · reuses `generateInviteToken` from `_shared/invite.ts` · validates `target_user_id` UUID → throws `BAD_USER_ID` · validates `ttl_hours` ∈ [1..168] → throws `BAD_TTL` · checks target profile exists (404 if missing) · INSERT via service_role with target_kind='password_reset' · **branches on Postgres 23505 unique_violation → 409 + `E_RES_CONFLICT` 「Friend already has a pending password reset」** (race-defense double-pending) ❸ `supabase/config.toml` 扩 — `[functions.admin-reset-password]` verify_jwt=true stanza ❹ `src/lib/admin/invite.ts` + `supabase/functions/_shared/invite.ts` (双 mirror 文件) extended — `InviteValidationCode` union 加 `BAD_USER_ID` (防 invite-flow naming 漂移到 reset) · `inviteErrorCode` switch 改 BAD_USER_ID → E_VAL_INVALID_FORMAT ❺ `src/lib/api/admin.ts` 扩 — `createPasswordReset({ targetUserId, ttlHours? })` · `CreatedPasswordReset` interface 全字段 ❻ `src/lib/api/admin.test.ts` rewritten (~280 行) — 36 cases (createInvite 14 + createPasswordReset 6 + createPasswordReset 错误映射 8 + mapAdminError 8) ❼ `src/lib/api/friends.ts` (NEW · ~50 行) — `listFriendsOfOwner()` RLS-aware PostgREST query (max 20 friends per F-CONV-01) ❽ `src/hooks/useFriendsQuery.ts` (NEW · ~80 行) — TanStack Query wrapper · 30s staleTime (matches useConversations) · null-owner renders empty ❾ `src/hooks/useFriendsQuery.test.tsx` (NEW · ~250 行 · 13 cases) — null owner/render gating · happy path w/ mockResolvedValue · RLS-filter ² · staleTime 30s ❿ `src/components/settings/PasswordResetCard.tsx` (NEW · ~310 行) — friend picker (select + i18n `settings.friend.empty` placeholder) + Generate button (int=accent, loading=`isCreating`) + Create pipeline + post-create success card w/ copy-to-clipboard + error strip w/ `codeToI18nKey` covers `{ BAD_USER_ID, BAD_TTL, MALFORMED_BODY, E_VAL_INVALID_FORMAT, E_VAL_REQUIRED_FIELD, E_RES_CONFLICT }` · `UseRequireRole('owner')` guard · data-testids × 5 ⓫ `src/components/settings/PasswordResetCard.test.tsx` (NEW · ~440 行 · 13 cases) — render gating · button disable/enable · create flow happy · error surfacing × 5 (BAD_USER_ID / BAD_TTL / E_VAL_INVALID_FORMAT / E_RES_CONFLICT / fallback) · copy-to-clipboard · done reset · role guard · friend-picker-while-loading · error retry post-error ⓬ `src/app/pages/SettingsAdminPage.tsx` — placeholder DISABLED replaced with `<PasswordResetCard />` ⓭ `src/app/pages/ResetPasswordPlaceholderPage.tsx` (NEW · ~30 行) — Friend-side stub route at `/reset-password/:token` (M6-4.1 friend-side EF deferred) · 30 天淡出提示 + OWNER 卡片镜像 ⓮ `src/app/routes.tsx` 扩 — `/reset-password/:token` route + import placeholder page (loose route — no RequireAuth guard · unauthenticated entry path for friend-side validation future M6-4.1) ⓯ i18n × 2 lang — `settings.passwordReset.{ sectionLabel, generate, copyUrl, copied, successCardTitle, error.{ invalidInput, networkError, alreadyPending, unexpected } }` + `settings.friend.{ placeholder, count_one, count_other }` + `resetPlaceholder.{ invalid, used, expired, ownerDeleted }` (5 sub-stubs) · **vitest M6-4 specs 13/13 + 84/84 spec runner + 360/360 full unit suite + 0 new tsc errors + reviewer ship-ready after 1 should-fix (E_RES_CONFLICT 23505 packaging)** · 本机 live verify 0 (per KI-9) · 云 端 EF deploy via `supabase functions deploy admin-reset-password --project-ref <cloud>` + migration `supabase db push --include-all` |
| M6-4.1 | EF `reset-password-complete` — Friend-side password reset completion (anonymous verify_jwt=false) | 4 files ship | ✅ 已完成 (S45.0 · 本机 static-only per KI-9) — ❶ `supabase/functions/reset-password-complete/index.ts` (NEW · ~140 行) — anonymous verify_jwt=false EF · validates token shape (32-char base64url) + password (≥8 chars) · looks up invites row · checks target_kind=password_reset, not expired, not used, not revoked · resolves target_user_id · updates password via `supabaseAdmin.auth.admin.updateUserById()` · marks invite as used (non-fatal if mark fails) · returns 200 { success, message } · error codes: E_VAL_INVALID_FORMAT / E_RES_NOT_FOUND / E_RES_TOKEN_EXPIRED / E_RES_TOKEN_USED / E_RES_TOKEN_REVOKED ❷ `supabase/config.toml` — `[functions.reset-password-complete]` verify_jwt=false stanza ❸ `src/lib/api/admin.ts` — `adminApi.resetPasswordComplete({ token, password })` method (reuses mapAdminError) ❹ `src/app/pages/ResetPasswordPlaceholderPage.tsx` — REPLACED stub with full password reset form: token validation · password+confirm form · client-side validation (min 8 chars, match) · submit to EF · success card with /login link · error strip with codeToI18nKey (6 codes) · invalid token card · `data-testid × 10` ❺ `src/app/pages/ResetPasswordPlaceholderPage.test.tsx` — 17 test cases (page chrome · invalid token ×3 · client validation ×2 · validation clear · successful submit ×2 · error states ×5 · submit gating ×2 · loading state) ❻ `src/lib/i18n/locales/{en,zh-CN}/translation.json` — added `resetComplete.*` section (~15 keys each: title, subtitle, newPassword, confirmPassword, passwordPlaceholder, submit, submitting, success.{title,message,login}, errors.{invalidToken,tokenNotFound,tokenExpired,tokenUsed,tokenRevoked,invalidInput,internal,unknown}) · **vitest 17/17 pass + 412/412 full suite pass (new +14 from M6-4 398 baseline) · 0 new tsc errors · reviewer LGTM** |
| M6-5 | EF `admin-delete-friend`（原子批量 left_at UPDATE） | CAP-20 / F-SEC-06 / BF-14 | ✅ 已完成 (S43.0 · 本机 static-only per KI-9) — atomic batch UPDATE via `fn_admin_delete_friend` RPC (SECURITY DEFINER · FOR UPDATE row lock · SET `profiles.deleted_at` + `conversation_members.left_at` in one transaction · idempotent re-call returns original `deleted_at` + `conversations_left=0`) · migration 0018 (NEW · ~90 行) `profiles.deleted_at` ADD COLUMN + partial indexes + `fn_admin_delete_friend` RPC + GRANT service_role · EF `admin-delete-friend` (JWT-verified · 3-layer defense: caller role check + target profile role check + RPC Owner-delete refusal) · `src/lib/api/admin.ts` `deleteFriend()` + `DeletedFriendSummary` · 15/15 vitest DeleteFriendCard specs + 24/24 ConfirmModal specs + 28 admin.test.ts cases · **33 files · 398 tests passed** + 0 new tsc errors + 1 reviewer should-fix (useCallback→useMemo in ConfirmModal.tsx) APPLIED |
| M6-6 | `confirm` modal（输 "confirm" 字才能 enable 提交） | F-SEC-06 / AC.18 | ✅ 已完成 (S43.0 · 本机 static-only per KI-9) — reusable `<ConfirmModal>` via `createPortal` · phrase gate (case-insensitive trim match) · Escape dismiss + Cancel button · backdrop click does NOT close (destructive modal UX) · `aria-modal="true"` + `aria-labelledby`/`describedby` + Tab-trap · `testIdPrefix` isolation · i18n `confirmModal.{confirmLabel,submitPlaceholder,phraseMatches}` × 2 lang |
| M6-7 | admin-create-invite UI 复制 URL 到剪贴板 + 微信分享引导 | F-AUTH-03 | ✅ 已完成 (S43.0 · 与 M6-5+M6-6 同期 ship) — `InviteNewPage.tsx` success card `<Button onClick={handleCopy}>` + `navigator.clipboard.writeText` + `execCommand('copy')` fallback + 2s "Copied!" feedback via `COPY_FEEDBACK_MS=2000` + `copyTimerRef` cleanup · 测试已覆盖 (copy section 2 cases) · 16/16 tests pass |
