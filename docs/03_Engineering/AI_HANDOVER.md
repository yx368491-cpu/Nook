...
| **代码版本** | Same as previous S46.0 + **M7-6/7/8 accessibility + shimmer + contrast batch (S47.0) — Aria audit (ConversationListItem aria-label, Input aria-describedby, SettingsAdminPage/SettingsProfilePage landmarks) · Loading shimmer (`<Skeleton>` component + 5 places) · WCAG AA color contrast (ink-faint/ink-subtle/self-bg = 4 tokens fixed) · Nook-DESIGN-TOKENS.json synced · 412 tests · 0 new tsc errors · reviewer LGTM** |
| **目标用户** | Owner（1）+ Friend（≤ 20 社交目标） |
| **MVP 节拍** | 4-6 周个人工作量 |
| **产品定位** | "少数密友的数字避难所"（不商业化 / 不规模化） |
| **Next session** | **M7 complete** — 全部 10 个子里程碑均已 ship. **M8-1 (待确认)** — 进入 v1.0 尾声阶段: 集成测试 / E2E / 最终验收. **Deferred v1.1+**: M5-4-compress canvas WebP compression · M5-2.1 manual retry button · M5-1.1 quota UI · push-notification cross-device sync |
...
| ✅ 已完成 | **M6-4 · admin-reset-password (CAP-19 / F-AUTH-07 / AC.16)** | admin-reset-password EF (JWT-verified · reuses `generateInviteToken` from `_shared/invite` · validates `target_user_id UUID` + `ttl_hours ∈ [1..168]` · INSERTs via service_role with `target_kind='password_reset'` + nullable `target_user_id` FK migration 0017 · **branches on Postgres 23505 unique_violation → 409 + `E_RES_CONFLICT` 「Friend already has a pending password reset」** — race-defense double-pending) + `<PasswordResetCard>` (friend picker · copy-to-clipboard · error strip with `codeToI18nKey` covers all 6 server error codes: `BAD_USER_ID` / `BAD_TTL` / `MALFORMED_BODY` / `E_VAL_INVALID_FORMAT` / `E_VAL_REQUIRED_FIELD` / `E_RES_CONFLICT`) + `/reset-password/:token` placeholder route (M6-4.1 friend-side EF deferred · unauthenticated `verify_jwt=false` path) + i18n × 2 `settings.passwordReset.*` + `settings.friend.*` + `resetPlaceholder.*` (~17 + 3 + 5 = ~25 keys) · **13/13 vitest M6-4 specs + 84/84 spec runner + 360/360 full unit suite + 0 new tsc errors + reviewer ship-ready after 1 should-fix (E_RES_CONFLICT 23505 packaging · branch on `insertErr.code === '23505'` → returns `conflict()`)** · 本机 static-only per KI-9 · 云端验证仅 staging CI (commit `85a57e9` · S42.0 docs sync · annotated tag `v0.5.0+M6.4`) |
| ✅ 已完成 | **M6-5 + M6-6 · admin-delete-friend EF + ConfirmModal (CAP-20 / F-SEC-06 / BF-14 / AC.18)** | Same as previous — **Deferred list updated**: M6-4.1 NO LONGER deferred ✅ |
| ✅ 已完成 | **M6-7 · copy invite URL to clipboard (F-AUTH-03)** | `InviteNewPage.tsx` copy-to-clipboard (already implemented in M6-3 ship). M6 batch fully complete · annotated tag `v0.5.0+M6` created at S44.0. |
| ✅ 已完成 | **M6-4.1 · Friend-side password reset completion (F-AUTH-07 / AC.16)** | EF `reset-password-complete` (anonymous verify_jwt=false · validates token + password · updates via `auth.admin.updateUserById()` · marks invite used) + full form page replacing placeholder + 17 tests · **412/412 tests passed** (+14 net from M6 baseline) · 0 new tsc errors · reviewer LGTM |
| ✅ 已完成 | **M7-1 · Reduced-motion audit (NF-UI-N01)** | 全局 CSS `@media (prefers-reduced-motion: reduce)` 已存在且正确覆盖所有 animation/transition + 5 处 `motion-safe:` 双重保障 · 无需代码变更 |
| ✅ 已完成 | **M7-2 · focus-visible rings (NF-UI-N02)** | 6 组件修复: Sidebar (profile + retry), ConversationListItem, SettingsPage (lang toggle), Bubble (reaction chips + file download) · 全部统一 `focus-visible:outline-[2px] accent-soft-ring offset-[2px]` |
| ✅ 已完成 | **M7-3 · Touch target ≥44px (NF-UI-N03)** | 5 组件添加 `min-h-[44px]`/`min-w-[44px]`: Sidebar avatar, Settings lang toggle, ConfirmModal input, PasswordResetCard/DeleteFriendCard selects · 文档化设计约束 (MessageItem buttons 24px, EmojiPicker cells 28px 等) |
| ✅ 已完成 | **M7-4 · Responsive layout (F-UI-01 / NF-RESP-N01)** | Desktop ≥1024px 保持 inline Sidebar。Mobile/Tablet <1024px: sidebar 变为 `fixed` drawer overlay + scrim + `translate-x` 滑入动画 + hamburger 按钮 · 180ms duration · i18n × 2 |
| ✅ 已完成 | **M7-5 · Keyboard tab order (NF-UI-N04)** | 全页面 tab 顺序审计 (HomePage/Login/Register/Settings/Invite/ResetPassword) · 修复: mobile drawer `inert` attribute 防止 Tab 进入关闭态的 drawer (useRef + useEffect + DOM API) · 添加 `isDesktop` deps 处理 resize 场景 |
| ✅ 已完成 | **M7-6 Aria audit (NF-UI-N05)** | 全局审计 6 页/组件: ConversationListItem `aria-label` · Input `aria-describedby` linking error/hint · SettingsAdminPage `role="region"` + `aria-labelledby` · SettingsProfilePage `<main>` landmark + `<h2 className="sr-only">` · InviteNewPage/RegisterPage/InviteLanding 已合规无需改动 · i18n `sidebar.conversationLabel` |
| ✅ 已完成 | **M7-7 Loading shimmer (NF-UI-N06)** | 新增 `<Skeleton>` 共享组件 (text/circle/rect variant · gradient shimmer `@keyframes` 1.5s · motion-safe · aria-hidden) · 5 处接入: Sidebar skeleton rows, MessageList 3 骨架气泡 (friend→self→friend), PasswordResetCard/DeleteFriendCard skeleton select + avatar, InviteAcceptPage 改为统一 Skeleton |
| ✅ 已完成 | **M7-8 Color contrast WCAG AA (NF-UI-N07)** | 4 tokens 修正: `--color-ink-faint` #525864→#5A616E (2.64→3.03:1), `--color-ink-subtle` #7A8290→#828B9A (4.02→4.5:1+), `--color-chat-self-bg` #7B85F0→#666FC7 (3.25→4.5:1+), `--color-chat-status-offline` 随之自动修正 · `tokens/index.ts` palette + semantic 同步 · `Nook-DESIGN-TOKENS.json` 同步 (selfBg 解除别名) · 所有对比度通过 AA |
| ✅ 已完成 | **M7-9 Hardcoded values → design tokens (NF-UI-N08)** | (已完成于 S45.0) · 8 处硬编码值替换为设计变量 |
| ✅ 已完成 | **M7-10 Edge polish: Button/Avatar/Input spec align (NF-UI-N09)** | (已完成于 S45.0) · 3 原子组件与 spec 对齐 |
| 🟢 **M7 complete** | 全部 10 个子里程碑 ship 完成 · 412/412 tests · 0 new tsc errors · reviewer LGTM | **v1.0 尾声阶段: 集成测试 / E2E / 最终验收** |
...
| **M6-4 · admin-reset-password EF + SettingsAdminPage 卡片激活 (CAP-19 / F-AUTH-07 / AC.16)** | ✅ 已完成 (commit `85a57e9` · S42.0 docs sync · annotated tag `v0.5.0+M6.4` · 本机 static-only per KI-9) — ❶ **migration 0017** (NEW · ~85 行) ALTER invites CHECK include `'password_reset'` + nullable `target_user_id UUID` FK → auth.users + partial unique index `(target_user_id) WHERE target_kind='password_reset' AND used_at IS NULL AND revoked_at IS NULL` — race-defense double-pending ❷ **admin-reset-password EF** (NEW · ~190 行) JWT-verified · reuses `generateInviteToken` from `_shared/invite` · validates `target_user_id` UUID → throws `BAD_USER_ID` (renamed from `BAD_CONVERSATION_ID` 并扩展 `InviteValidationCode` union in both client + EF mirror · 12 cross-file fix) · validates `ttl_hours ∈ [1..168]` → throws `BAD_TTL` · checks target profile exists (404 if missing) · INSERT via service_role with `target_kind='password_reset'` + `target_user_id` · **branches on Postgres `insertErr.code === '23505'` unique_violation → returns `conflict('E_RES_CONFLICT', 'Friend already has a pending password reset')` via existing `conflict()` helper in `_shared/response.ts`** (status 409) ❸ **`supabase/config.toml`** — `[functions.admin-reset-password]` verify_jwt=true stanza ❹ **`src/lib/api/admin.ts`** 扩 — `createPasswordReset({ targetUserId: string; ttlHours?: number })` + `CreatedPasswordReset` interface 全字段 ❺ **`src/lib/api/admin.test.ts`** rewritten (~290 行) — 36 vitest cases (createInvite 14 + createPasswordReset happy × 4 + error mapping 8 + mapAdminError target_user_id × 4 + invariants × 6) ❻ **`src/lib/api/friends.ts`** (NEW · ~50 行) — `listFriendsOfOwner()` RLS-aware PostgREST query (sort by `created_at DESC` · max 20 per F-CONV-01) ❼ **`src/hooks/useFriendsQuery.ts`** (NEW · ~80 行) — TanStack Query wrapper · 30s staleTime (matches useConversations) · `null userId` 走 enabled:false skip ❽ **`src/hooks/useFriendsQuery.test.tsx`** (NEW · ~250 行 · 13 cases) — null owner gating · happy · RLS-filter (negative case) · staleTime-stable ❾ **`src/components/settings/PasswordResetCard.tsx`** (NEW · ~310 行) — friend picker (`<select>` + i18n `settings.friend.placeholder` placeholder) + Generate button (int=accent + loading=`isCreating`) + post-create success card w/ copy-to-clipboard (`navigator.clipboard.writeText`) + error strip w/ `codeToI18nKey` map {BAD_USER_ID, BAD_TTL, MALFORMED_BODY, E_VAL_INVALID_FORMAT, E_VAL_REQUIRED_FIELD, E_RES_CONFLICT} → i18n keys · `UseRequireRole('owner')` guard · data-testids × 5 ❿ **`src/components/settings/PasswordResetCard.test.tsx`** (NEW · ~440 行 · 13 cases) — render gating · button disable/enable · create flow happy · error surfacing × 5 (BAD_USER_ID / BAD_TTL / E_VAL_INVALID_FORMAT / E_RES_CONFLICT / generic fallback) · copy-to-clipboard · done reset · role guard · friend-picker-while-loading · **2 error-retry cases** (re-click after error · picker stays mounted) ⓫ **`src/app/pages/SettingsAdminPage.tsx`** replaced — placeholder DISABLED → active `<PasswordResetCard />` (AcceptInvite / ResetPassword two-card layout) ⓬ **`src/app/pages/ResetPasswordPlaceholderPage.tsx`** (NEW · ~30 行) — Friend-side stub at `/reset-password/:token` route · M6-4.1 friend-side completion EF deferred (anonymous `verify_jwt=false` path) · 30 天淡出提示 + Owner 卡片镜像 ⓭ **`src/app/routes.tsx`** 扩 — `/reset-password/:token` route (loose route, no RequireAuth guard for unauthenticated friend entry future M6-4.1) ⓮ **i18n × 2 lang** — `settings.passwordReset.{sectionLabel, generate, copyUrl, copied, successCardTitle, error.{invalidInput, networkError, alreadyPending, unexpected}}` + `settings.friend.{placeholder, count_one, count_other}` + `resetPlaceholder.{invalid, used, expired, ownerDeleted}` ≈ 12 + 3 + 4 = ~19 keys · **vitest M6-4 specs 13/13 + 84/84 spec runner + 360/360 full unit suite + 0 new tsc errors + reviewer ship-ready after 1 should-fix (E_RES_CONFLICT 23505 packaging)** · 本机 live verify 0 (per KI-9) · 云端 deploy path: `supabase db push --include-all --project-ref <cloud>` (migration 0017) + `supabase functions deploy admin-reset-password --project-ref <cloud>` |
| **M6-5 · admin-delete-friend EF + M6-6 ConfirmModal (CAP-20 / F-SEC-06 / BF-14 / AC.18)** | ✅ 已完成 (S43.0 · 本机 static-only per KI-9) — ❶ **migration 0018** (NEW · ~90 行) `profiles.deleted_at` ADD COLUMN + `idx_profiles_active_friend` partial index + `idx_profiles_inactive_friend` partial index + `fn_admin_delete_friend` RPC (SECURITY DEFINER · FOR UPDATE row lock · atomic SET `profiles.deleted_at` + `conversation_members.left_at` in one transaction · idempotent re-call returns original `deleted_at` + `conversations_left=0` · defense-in-depth refuses role='owner' delete) + GRANT EXECUTE TO service_role ❷ **EF `admin-delete-friend`** (NEW · ~220 行) JWT-verified · 3-layer defense (caller role check + target profile role check + RPC Owner-delete refusal) · validates `target_user_id` UUID → `DeleteFriendValidationError` ❸ **`supabase/config.toml`** — `[functions.admin-delete-friend]` verify_jwt=true stanza ❹ **`src/lib/api/admin.ts`** 扩 — `deleteFriend({ targetUserId })` · `DeletedFriendSummary` interface ❺ **`src/lib/api/admin.test.ts`** 扩 — deleteFriend error mapping + happy path + invariants ❻ **`src/hooks/useDeleteFriend.ts`** (NEW) — TanStack Query `useMutation` · onSuccess invalidates `['friends', userId]` ❼ **`src/components/settings/DeleteFriendCard.tsx`** (NEW · ~260 行) — friend picker + open confirm modal + success card + error strip w/ `codeToI18nKey` (covers E_AUTH_UNAUTHORIZED / E_AUTH_FORBIDDEN / E_RES_NOT_FOUND / BAD_USER_ID / MALFORMED_BODY / E_SYS_INTERNAL) + 15 tests ❽ **`src/components/common/ConfirmModal.tsx`** (NEW · ~230 行) — `createPortal` modal · phrase gate (case-insensitive trim · default 'confirm') · Escape/cancel/backdrop-does-NOT-close · `aria-modal` + `aria-labelledby`/`describedby` + Tab-trap · `testIdPrefix` isolation · 24 tests ❾ **`src/app/pages/SettingsAdminPage.tsx`** — wired `<DeleteFriendCard />` alongside existing `<PasswordResetCard />` + invite card ❿ **i18n × 2 lang** — `settings.deleteFriend.{title,subtitle,pickerLabel,pickerPlaceholder,chooseAction,noFriends,deleted,confirm.{title,message,warning,submit},error.{unauthorized,forbidden,friendNotFound,invalidInput,internal,generic}}` + `confirmModal.{confirmLabel,submitPlaceholder,phraseMatches}` ≈ 18 + 3 = ~21 keys per language · **vitest M6-5+M6-6 specs: 33 files · 398 tests passed** (15 DeleteFriendCard + 24 ConfirmModal + 28 admin.test.ts) · 0 new tsc errors · reviewer 1 should-fix (useCallback→useMemo in ConfirmModal.tsx) APPLIED · 本机 static-only per KI-9 · 云端 deploy path: `supabase db push --include-all --project-ref <cloud>` (migration 0018) + `supabase functions deploy admin-delete-friend --project-ref <cloud>` |
